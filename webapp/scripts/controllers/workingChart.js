  ...
  .controller('DataController', function ($scope, VirtualSensorService, SettingsService, $http) {
    ...
    function getData(sensorResult) {
      ...
      for(var i = sensorResult.tuples.length - 1; i >= 0; --i)
      {
        ...
        //alert("Date: " + data[sensorResult.header[1]] + " Value: " + data[sensorResult.header[0]]);
        var date = data[sensorResult.header[1]].substring(0,10).split("-");
        var time = data[sensorResult.header[1]].substring(11,18).split(":");
        var firstDate = new Date(Date.UTC(date[0], date[1], date[2], time[0], time[1]));

        chartData.push({
          x: firstDate,
          y: parseFloat(data[sensorResult.header[0]])
          //name: $scope.selectedChart.name
        });
      }
    ...
    $scope.showResulChart = function(name) {
      getData($scope.selectedChart);
